...
do
    case $mysql_v in
        5.1)
            ...
            tar zxvf mysql-5.1.72-linux-$ar-glibc23.tar.gz
            check_ok
        ...
        5.6)
            ...
            tar zxvf mysql-5.6.26-linux-glibc2.5-$ar.tar.gz
            check_ok
            [ -d /usr/local/mysql ] && /bin/mv /usr/local/mysql /usr/local/mysql_`date +%s`
...
do
    case $php_v in
        5.4)
            ...
            tar jxvf php-5.4.45.tar.bz2 && cd php-5.4.45
        ...
        5.6)
            ...
            tar zxvf php-5.6.6.tar.gz &&   cd php-5.6.6
            for p in openssl-devel bzip2-devel \
            libxml2-devel curl-devel libpng-devel \
            libjpeg-devel freetype-devel libmcrypt-devel\
            libtool-ltdl-devel perl-devel
